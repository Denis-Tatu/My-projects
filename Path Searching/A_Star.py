...
class Node:
    ...
    def __lt__(self, other):
        ...
        nodes and since this algorithm uses a priority queue, the comparison is based on the costs between states and the heuristic costs"""
    ...
    def __str__(self):
        ...
        s = ""
        s += f"State: {self.state}, Path: {self.get_path()}, Total Cost: {self.cost + self.heuristic_cost}"
        return s
    ...
    def generate_successors(self, n: Node) -> list[Node]:
        ...
        for i in range(l):
            if self.matrix[n.index][i] != 0:
                s = Node(i, states[i], n, states[i], n.level + 1, 0, n.cost + self.distance_matrix[n.index, i],
                         self.heuristic(i, self.goal_state))
                succ.append(s)
        return succ

    def a_star_search(self, nsol: int):
        """BFS based algorithm that uses a priority queue to explore nodes, prioritizing nodes with the lowest estimated total cost.
    The total cost is the sum of the actual cost to reach the node and the heuristic estimate of the cost to reach the goal.
    The algorithm efficiently guides the search towards the goal by combining these costs."""
        ...
        nodes = 0
        number_solutions = 0
        start = Node(self.start_index, states[self.start_index], None, "", 0, 0, 0,
                     self.heuristic(self.start_index, self.goal_state))
        ...
        while not frontier.empty():
            ...
            if current.state == goal:
                number_solutions += 1
                message += "A* Search: SUCCESS\n"
                message += current.get_path() + "\n"
                message += "Total Cost: " + str(current.cost) + "\n"
        ...
        message += "A* Search: FAILURE - There is no path\n"
        message += "Nodes created by A* Search: " + str(nodes) + "\n"

        return message, current.get_path()

    def heuristic(self, current_index, goal_state):
...
class GraphSearch:
    def __init__(self, root):
        self.root = root
        self.root.title("Route search with A*")
    ...
    def start_search(self):
        ...
        message, path = p.a_star_search(num_solutions)
